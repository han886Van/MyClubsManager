      ...
      <div class="member" v-show="userRole==1">
        ...
        <ul class="list">
          ...
          <li class="societyList" v-for="(item,index) in associationList">
            <span @click="toRouter('/news',item.association_id)">{{index+1}}</span>
            <span @click="toRouter('/news',item.association_id)">{{item.association_id}}</span>
            <span @click="toRouter('/news',item.association_id)" v-show="item.type_id==1">专业学术类</span>
            <span @click="toRouter('/news',item.association_id)" v-show="item.type_id==2">科技创新类</span>
            <span @click="toRouter('/news',item.association_id)" v-show="item.type_id==3">艺术兴趣类</span>
            <span @click="toRouter('/news',item.association_id)" v-show="item.type_id==4">体育健身类</span>
            <span @click="toRouter('/news',item.association_id)" v-show="item.type_id==5">公益服务类</span>
            <span @click="toRouter('/news',item.association_id)">{{item.name}}</span>
            <span @click="toRouter('/news',item.association_id)">{{item.user_name}}</span>
            <span @click="toRouter('/news',item.association_id)">{{item.place}}</span>
            <span @click="toRouter('/news',item.association_id)">{{item.person_num}}</span>
            <div>
              <span @click="toRouter('/news',item.association_id)" class="blue">查看社团新闻</span>
            </div>
          </li>
